              ...
              return (
                <MyPageMobileCard
                  ...
                  actionButton={(
                    <>
                      {p.tradeSn && (
                        <button type="button" onClick={() => onOpenTradeDetail ? onOpenTradeDetail(p.tradeSn) : navigate(`/trades/${p.tradeSn}/seller`)} className="btn btn-sm btn-primary">거래 관리</button>
                      )}
                      {!p.tradeSn && isActive && (
                        <button type="button" onClick={() => navigate(`/product/${p.prdSn}/seller`)} className={`btn btn-sm ${p.aucStatusCd === 'AUCC0005' ? 'btn-ghost' : 'btn-primary'}`}>
                          {p.aucStatusCd === 'AUCC0005' ? '취소 상품 보기' : '판매 관리'}
                        </button>
                      )}
                      {isDraft && (
                        <button type="button" onClick={() => navigate('/product/register', { state: { prdSn: p.prdSn } })} className="btn btn-sm btn-ghost">등록재개</button>
                      )}
                      {!p.tradeSn && isEnded && (
                        <button type="button" onClick={() => navigate(`/product/${p.prdSn}/seller`)} className="btn btn-sm btn-ghost">판매 기록</button>
                      )}
                      {(isDraft || isEnded) && !p.tradeSn && p.aucStatusCd !== 'AUCC0003' && (
