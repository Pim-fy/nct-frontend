...
export const SERVICE_MENU_ITEMS = [
  { label: "청소", left: 322, image: assets.image10, imgLeft: 373, imgTop: 1053, imgW: 53, imgH: 47 },
  { label: "이사", left: 482, image: assets.image11, imgLeft: 535, imgTop: 1055, imgW: 52, imgH: 42 },
  { label: "설치/수리", left: 642, image: assets.image8, imgLeft: 696, imgTop: 1054, imgW: 48, imgH: 45 },
  { label: "인테리어", left: 802, image: assets.image9, imgLeft: 853, imgTop: 1051, imgW: 45, imgH: 50 },
  { label: "레슨", left: 962, image: assets.image7, imgLeft: 1007, imgTop: 1052, imgW: 57, imgH: 49 },
];
  ...
  return (
    <section className="absolute contents left-[167px] top-[867px]" data-name="SECTION_2(서비스메뉴/HOTITEM)">
      ...
      <div className="absolute contents left-[167px] top-[1015px]" data-name="서비스_icon">
        ...
        {SERVICE_MENU_ITEMS.map((item) => (
          <div key={item.label} className="absolute contents" style={{ left: item.left, top: 1090 }}>
            ...
            <p
              className="-translate-x-1/2 absolute font-['Noto_Sans_KR:Medium'] font-medium leading-[normal] text-[16px] text-black text-center top-[1116px] tracking-[-1.28px] whitespace-nowrap"
      ...
      <div className="absolute contents left-[1340px] top-[867px]" data-name="HOT ITEM">
        ...
        <MoreButton left={1614} top={897} variant="dark" />

        <div className="absolute contents left-[1360px] top-[957px]" data-name="num">
          ...
          <div className="absolute font-['Noto_Sans_KR:Bold'] font-bold leading-[0] left-[1371px] text-[13px] text-white top-[957px] tracking-[-0.65px] whitespace-nowrap">
            {HOT_ITEMS.map((item, i) => (
        ...
        <div className="absolute font-['Noto_Sans_KR:Regular'] font-normal leading-[0] left-[1403px] text-[16px] text-black top-[957px] tracking-[-0.8px] whitespace-nowrap">
          {HOT_ITEMS.map((item, i) => (
            <p key={item.rank} className={`leading-[50px] ${i < HOT_ITEMS.length - 1 ? "mb-0" : ""} whitespace-pre`}>{` ${item.name} `}</p>
          ))}
        </div>
        <div className="absolute -translate-x-full font-['Noto_Sans_KR:Bold'] font-bold leading-[0] left-[1741px] text-[16px] text-black text-right top-[957px] tracking-[-0.8px] whitespace-nowrap">
          {HOT_ITEMS.map((item, i) => (
            <p key={item.rank} className={`leading-[50px] ${i < HOT_ITEMS.length - 1 ? "mb-0" : ""} whitespace-pre`}>{`${item.price} `}</p>
          ))}
        </div>

        <div className="absolute left-[1361px] top-[1006px] w-[380px] flex flex-col">
          {[0,1,2,3].map(i => (
