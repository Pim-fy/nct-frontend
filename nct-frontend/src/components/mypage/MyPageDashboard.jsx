...
// - 프로필 요약/알림/통계 카드/오늘 확인할 일/관심상품은 여러 도메인(포인트,경매,서비스,관심상품)의
//   데이터를 한 화면에 모아 보여주는 대시보드라, 해당 도메인 API가 모두 붙기 전까지는 정적 데이터로 구성한다.
// TODO: 포인트(F-PNT), 경매(F-AUC), 서비스거래(F-SVC), 관심상품(F-WISH) API가 준비되면
//       아래 STAT_CARDS / TODAY_ITEMS / WISH_ITEMS 를 각 도메인 조회 결과로 교체한다.
import React from "react";
import { User, Wallet, Gavel, Wrench, PackageCheck, Plus } from "lucide-react";
...
function StatCard({ color, icon: Icon, label, value, unit, meta }) {
  return (
    <div
    ...
    >
      ...
      <Icon className="absolute left-[24px] top-[37px] opacity-90" size={40} />
      ...
      <button
        ...
        className="absolute right-[18px] top-[18px] bg-transparent border-none cursor-pointer text-white"
  ...
  return (
    <div className="relative w-[650px] h-[492px] bg-white border border-[rgba(0,0,0,0.11)] rounded-[15px] overflow-hidden shrink-0">
      ...
      <button
        type="button"
        onClick={() => toast({ icon: "info", title: "준비 중인 기능입니다." })}
        className="absolute right-[19px] top-[31px] bg-transparent border-none cursor-pointer text-[#3a3a3a]"
...
export default function MyPageDashboard({ user }) {
  ...
  return (
    <div className="absolute contents left-[448px] top-[167px]" data-name="CONTENTS">
      {/* 프로필 요약 */}
      <div className="absolute left-[448px] top-[167px] size-[72px] rounded-full bg-[#e6f0ff] flex items-center justify-center">
        <User className="text-[#0064ff]" size={36} />
      </div>
      ...
      <div className="absolute left-[448px] top-[247px] flex gap-2">
        <button
          type="button"
          className="h-[38px] w-[100px] rounded-[19px] border border-[#d9d9d9] bg-white text-[#4e4e4e] text-[14px] font-['Noto_Sans_KR:Regular'] cursor-pointer hover:bg-[#f5f5f5] transition-colors"
        >
          로그아웃
        </button>
        <button
          ...
          className="h-[38px] w-[113px] rounded-[19px] border border-[#d9d9d9] bg-white text-[#4e4e4e] text-[14px] font-['Noto_Sans_KR:Regular'] cursor-pointer hover:bg-[#f5f5f5] transition-colors"
        >
          제공자 전환
        </button>
      </div>
