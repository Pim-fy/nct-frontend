...
const ProviderApplyPage = () => {
  ...
  const categoriesQuery = useQuery({
    ...
    queryFn: () => getCategories(SERVICE_DOMAIN_CD).then((response) => response.data.filter((category) => category.catParentSn !== null)),
  });

  const applications = useMemo(() => applicationsQuery.data ?? [], [applicationsQuery.data]);
  const states = useMemo(() => Object.fromEntries(applications.map((item) => [item.categorySn, statusLabel(item.statusCode, item.statusName)])), [applications]);
  ...
  const uploadDocument = async (category, document, file) => {
    ...
    try {
      ...
      setUploadedFiles((current) => ({
        ...current,
        [key]: { categorySn: category.catSn, fileTypeCode: document.code, flSn: uploaded.flSn, fileName: file.name },
      }));
    } catch (uploadError) {
  ...
  const next = async () => {
    ...
    if (selectedCategories.some((category) => !Object.values(uploadedFiles).some((file) => file.categorySn === category.catSn))) {
    ...
    try {
      await submitMutation.mutateAsync({
        ...
        reason: '제공자 권한 신청',
        files: Object.values(uploadedFiles).filter((file) => selectedCategories.some((category) => category.catSn === file.categorySn))
  ...
  return <ContentPageShell className="provider-apply-page">
    ...
    <ContentPageHeader eyebrow="담당자 7 · F-PROV-002/003/006/007" title="제공자 권한 신청" description="여러 서비스 카테고리를 함께 선택할 수 있으며, 제출 뒤에는 카테고리별로 따로 심사됩니다." />
    <section className="provider-status-preview"><span>내 카테고리별 신청 현황</span><h2>{applications.length ? `${applications.length}건` : '미신청'}</h2><p>{applications.length ? '심사 결과와 반려 사유는 카테고리별로 확인할 수 있습니다.' : '첫 제공 카테고리를 선택해 신청해 주세요.'}</p>{applications.length > 0 && <dl>{applications.map((item) => <div key={item.applicationSn}><dt>{item.categoryName}</dt><dd>{statusLabel(item.statusCode, item.statusName)}{item.rejectReason && ` · ${item.rejectReason}`}</dd></div>)}</dl>}</section>
    <section className="provider-apply-card">
      <div className="provider-apply-notice"><strong>복수 카테고리 신청</strong><span>선택한 카테고리는 각각 별도 신청번호와 서류 묶음으로 관리자에게 전달됩니다.</span></div>
      <div className="provider-apply-steps">{STEPS.map((item, index) => <span className={index === step ? 'active' : index < step ? 'done' : ''} key={item}>{item}</span>)}</div>
      ...
      {step === 2 && <div className="provider-apply-files">{selectedCategories.map((category) => <div className="provider-apply-file-group" key={category.catSn}><strong>{category.catNm} 신청 서류</strong>{DOCUMENTS.map((document) => {
        ...
        return <label className="provider-apply-upload" key={document.code}>{document.label}<span>{uploaded ? uploaded.fileName : uploadingKey === key ? '업로드 중...' : '이미지 파일 선택'}</span><input accept="image/*" disabled={uploadingKey === key || submitMutation.isPending} onChange={(event) => uploadDocument(category, document, event.target.files?.[0])} type="file" /></label>;
      })}</div>)}</div>}
      {error && <p className="provider-apply-error">{error}</p>}
      <div className="provider-apply-actions"><button className="btn btn-outline" disabled={step === 0 || submitMutation.isPending} onClick={() => setStep(step - 1)} type="button">이전</button><button className="btn btn-primary" disabled={submitMutation.isPending || (step === 0 && categoriesQuery.isLoading)} onClick={next} type="button">{step === 2 ? `${selectedCategories.length}개 카테고리 신청하기` : '다음'}</button></div>
    </section>
  </ContentPageShell>;
};
